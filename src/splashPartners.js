...
const splashClient = new wallet_proto.WalletSplashThePot(
    walletUrl,
    credentials,
);

console.log(
    `Wallet client connected to ${walletUrl} (Transport Security: ${useTransportSecurity})`,
);
...
async function makeGrpcCallForSplashPartnerReconciliation(splashPayload) {
    ...
    return new Promise((resolve, reject) => {
        splashClient.splashShareableCredit(splashPayload, (error, response) => {
            if (error) {
...
async function main() {
    const data = await fsp.readFile(
        __dirname + "/json/splash_test.json",
        "utf8",
    );
    // const data = await fsp.readFile(__dirname + "/json/splashShareable/splash3_partner_ballebaazi_till_06-06-2025.json", "utf8");
    ...
    for (const item of payload) {
        ...
        const payload1 = (item.PAYLOAD);
        console.log(payload1);
        // await makeGrpcCallForSplashPartnerReconciliation(payload1);
